...
export default function ProductCatalog() {
  ...
  return (
    <section id="urunler" className="py-12 md:py-16 lg:py-20 bg-sand-50">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 md:mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-sage-800 mb-4 md:mb-0">
            Ürünlerimiz
          </h2>
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex-grow md:flex-grow-0">
              ...
              <Input
                ...
                className="pl-10 pr-4 py-2 w-full md:w-64 rounded-md border-sand-200 focus:border-sage-500 focus:ring-sage-500 bg-white"
              />
              {searchTerm && (
                <Button
                  variant="ghost"
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {displayedProducts.map((product) => (
                    <Card
                      key={product.id}
                      className="overflow-hidden border-sand-200 hover:shadow-xl transition-shadow duration-300 ease-in-out flex flex-col cursor-pointer bg-white"
                      onClick={() => handleProductClick(product)}
                    >
                      <CardHeader>
                        <CardTitle className="text-lg font-semibold text-sage-900 group-hover:text-sage-700">
                          {toTitleCase(product.name)}
                        </CardTitle>
                        {product.refNumber && (
                          <CardDescription className="text-xs text-clay-600">
                      ...
                      <CardContent className="flex-grow">
                        ...
                        {product.boxQuantity && (
                          <p className="text-xs text-gray-500 mt-2">Kutu İçi: {product.boxQuantity} adet</p>
                        )}
                      </CardContent>
                    </Card>
                  ))}
                ...
                {(canShowMore || canShowLess) && (
                  <div className="mt-8 flex justify-center gap-4">
                    {canShowLess && (
                      <Button variant="outline" onClick={handleShowLessProducts} className="font-sans border-sand-200 text-sage-700">
                        <ChevronUp className="mr-2 h-4 w-4" /> Daha Azını Göster
                      </Button>
                    )}
                    {canShowMore && (
                      <Button onClick={handleShowMoreProducts} className="font-sans btn-green-gradient">
